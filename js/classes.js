...
class Bringer_Before_After {
  constructor($obj = null) {
    ...
    this.$el = {
      ...
      $divider: jQuery(
        '<div class="bringer-before-after-divider">\
...
class Bringer_Infinite_List {
  constructor($obj = null, cfg = {}) {
    ...
    // Hover Event
    this.$el
      .on('mouseenter', '.bringer-infinite-list-item', function () {
    ...
    this.$list.children().each(function (i) {
      ...
      if (_self.first_circle) {
        ...
        if ('video' === type) {
          ...
          _self.$bg_wrap.append(
            `<div class="bringerl-bg-preview-item${i === 0 ? ' is-active' : ''}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`,
          );
        }
      }
    });
...
class Bringer_Marquee {
  constructor($obj = null) {
    ...
    this.init_speed =
      this.$marquee.attr('data-type') !== undefined ? parseInt(this.$marquee.attr('data-speed'), 10) : 5000;
...
class Bringer_Masked {
  ...
  get_path() {
    let path = ``,
      ...
      th = this.$el.height(),
      br = parseInt(
        this.$el.css('--masked-border-radius') !== undefined ? this.$el.css('--masked-border-radius') : this.cfg.dbr,
        10,
      ),
      ibr = parseInt(
        this.$el.css('--masked-inner-radius') !== undefined ? this.$el.css('--masked-inner-radius') : br,
        10,
      ),
      d_rect = {},
      pos = '';
